...
//                }
                return arr.push({
					"width":200,
                    "src": node.src,
                    "data_id":  (Upload.showCount - 1).toString()
                });
            });
    ...
    var Upload = {
        ...
        replaceImageUrl: function(url){
            ...
            i = 0, len = imgs.length, j = 0,
            myEditor = Upload.editor.textarea,
            //myEditor = document.querySelector('#' + UM.umId),
            str = myEditor.value;
			
            // 替换UM的innerHTML
            for(; i < len; i++){
				if( imgs[i].id != '' ){
					str = str.replace( imgs[i].src, url[imgs[i].id] );
					++j;
				}
            }
            ...
            myEditor.value = str;
            // console.log(str)
			document.querySelector('textarea[name="'+UM.areaName+'"]').value = str
